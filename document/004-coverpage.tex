	...
	\begin{axis}[
		...
		\begin{scope}[rotate around={\rot:(0,0)}]
			...
			\begin{scope}[color=BrickRed!10]
				...
				\draw (x4) -- (x4r) {};
				\draw (x5) -- (x5l) {};
				\node[anchor=north] at ($(x3)!0.5!(x3l)$) {$s_3$};
				\node[anchor=north] at ($(x4)!0.5!(x4r)$) {$s_4$};
				\node[anchor=north] at ($(x5)!0.5!(x5l)$) {$s_5$};
			\end{scope}

			\draw[<->,BrickRed!5,line width=2pt] (-44,\sep) -- (-44,-\sep);
			\node[BrickRed!9,anchor=south,rotate=\rot+90] at (-44.25,0) {margin\;\; = $\frac{2}{\lVert w \rVert}$};
		...
		\end{scope}
		\node[BrickRed!5,anchor=south east,rotate=\rot] at (p1) {$w^T x + \gamma = 1$};
		\node[BrickRed!5,anchor=south east,rotate=\rot] at (p0) {$\pi:\,w^T x + \gamma = 0$};
		\node[BrickRed!5,anchor=south east,rotate=\rot] at (p-1) {$w^T x + \gamma = -1$};
	\end{axis}
